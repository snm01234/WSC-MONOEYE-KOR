#!/usr/bin/env python3
"""Build follow-up Garrod/Loran/Guen literal retranslation from the validated candidate.

Parent is the user-validated first candidate ROM. New curated lines only; legacy
quality/sheet Korean remains forensic. Same install modes as the base builder.
"""
...
MAIN = ROOT / "out/patch/garrod_loran_guen_literal_retranslation_candidate.wsc"
MAIN_SAVE = ROOT / "sram/monoeye_ko_expanded.sav"
ORIGINAL = ROOT / "SD Gundam G Generation Mono-Eye Gundams.wsc"
SPEC = ROOT / "data/garrod_loran_guen_literal_retranslation_followup_ko.json"
TBL_PATH = ROOT / "out/patch/hangul_patch_pad3.tbl"
OUT_ROM = ROOT / "out/patch/garrod_loran_guen_literal_retranslation_followup_candidate.wsc"
OUT_SAVE = ROOT / "sram/garrod_loran_guen_literal_retranslation_followup_candidate.sav"
REPORT = ROOT / "out/patch/garrod_loran_guen_literal_retranslation_followup_candidate_report.json"

EXPECTED_MAIN_SHA256 = "0ee3f24b4d304eda7fc9b9290c40f383aeff08235d88d2fdd39cd3b935750adb"
...
def main() -> int:
    ...
    for row in sorted(entries, key=lambda item: int(str(item["abs"]), 16)):
        ...
        item: dict[str, Any] = {
            ...
            "body": body,
            "terminator": term,
            "reason": str(row.get("reason") or "literal retranslation"),
        }
    ...
    for row in prepared:
        # Neighbor terminator+1 through next few control bytes must stay put.
    ...
    report = {
        "schema_version": 1,
        "generated_by": "tools/build_garrod_loran_guen_literal_retranslation_followup_candidate.py",
        "status": "ready_for_runtime_validation",
        "parent_note": "user-validated first candidate; main TIP not yet promoted",
        "source_policy": {
