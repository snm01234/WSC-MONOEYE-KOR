#!/usr/bin/env python3
"""Compact overflowing ID-command effect help lines to 20 cells.

Japanese originals in this box are at most 20 cells.  The HUD prefixes
``소모`` plus a 1-2 digit SP cost and a separator space (5 cells worst case),
so the combined visible line is 25 cells.  Only aux records in
``5CBBB8-5CD748`` whose current Korean exceeds 20 cells are rewritten onto
new private ext3 slots.  Record length, prefix and NUL stay fixed.
...
OUT_ROM = ROOT / "out/patch/id_command_effect_width_candidate.wsc"
OUT_SAVE = ROOT / "sram/id_command_effect_width_candidate.sav"
REPORT = ROOT / "out/patch/id_command_effect_width_candidate_report.json"
...
def prepare_rows(parent: bytes, tbl: Tbl) -> list[dict[str, Any]]:
    ...
    for source in catalog.get("records") or []:
        ...
        if not ko or any(is_japanese_character(ch) for ch in ko) or len(ko) > MAX_CELLS:
            raise BuildError(f"invalid compacted Korean at {address}: {ko!r}")
        rows.append(
            {
                "abs": address,
                "logical": logical,
...
def main() -> int:
    ...
    if len(unique) != EXPECTED_UNIQUE:
        raise BuildError(f"unique phrase count drifted: {len(unique)}")
    ...
    ext3_write, _guard = write_ext3_slots_guarded(
        ...
        justification="candidate-bound ID-command effect help width compaction",
    )
    ...
    for row in rows:
        ...
        if actual != row["ko"] or len(actual) > MAX_CELLS or candidate[start + row["payload_len"]] != 0:
