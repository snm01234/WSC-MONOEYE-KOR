#!/usr/bin/env python3
"""Build a non-promotable candidate for exact native stock special-route rows."""
...
STORAGE = ROOT / "out/script/special_route_storage_static_audit.json"
OUT_ROM = ROOT / "out/patch/special_native_stock_static_candidate.wsc"
OUT_REPORT = ROOT / "out/patch/special_native_stock_static_candidate.json"
OUT_MANIFEST = ROOT / "out/patch/special_native_stock_static_candidate_contracts.json"
...
def main() -> int:
    ...
    by_address = {str(row.get("address") or "").upper(): row for row in contracts}
    ...
    for item in storage.get("rows") or []:
        if item.get("decision") != "native_stock_token_candidate_structural_hold":
            continue
        ...
        try:
            ...
            rejected.append({"abs": address, "reason": "invalid_token_hex"})
            continue
        if not contract:
            rejected.append({"abs": address, "reason": "contract_missing"})
            continue
        capacity = int(contract.get("body_capacity") or 0)
        if not token or b"\x00" in token or len(token) > capacity:
            rejected.append({"abs": address, "reason": "token_capacity_or_nul"})
            continue
        ...
        prefix = bytes.fromhex(
            str(contract.get("metadata_hex") or "")
            + str(contract.get("control_prefix_hex") or "")
        )
        ...
        if any(offset in touched for offset in range(start, end)):
            rejected.append({"abs": address, "reason": "overlap"})
            continue
        ...
        if before_boundary != after_boundary:
            ...
            rejected.append({"abs": address, "reason": "extent_or_boundary_change"})
            continue
    ...
    report = {
        "schema_version": 1,
        "generated_by": "tools/build_special_native_stock_static_candidate.py",
        "read_only": True,
        "runtime_trace": "stopped_by_user",
        "runtime_validation_performed": False,
        "promotion_allowed": False,
        "promotion_block_reason": "special-route native stock candidate only; runtime confirmation and user approval absent",
        "main_unchanged": sha(MAIN.read_bytes()) == sha(main_bytes),
        "selected": selected,
        "rejected": rejected,
        "counts": {
            "storage_rows_considered": sum(item.get("decision") == "native_stock_token_candidate_structural_hold" for item in storage.get("rows") or []),
