#!/usr/bin/env python3
"""Promote the ID-command effect width compaction to the main TIP.

ROM-only.  Live SaveRAM is preserved.
"""
...
CAND = PATCH / "id_command_effect_width_candidate.wsc"
BUILD = PATCH / "id_command_effect_width_candidate_report.json"
AUDIT = PATCH / "id_command_effect_width_candidate_audit.json"
...
PROMOTION = PATCH / "id_command_effect_width_promotion_report.json"
...
EXPECTED_CAND = "7da763eb36fe86c3f0741459246098dbab3b3a49a8a76a95e9d842dc25857397"
EXPECTED_CHECKSUM = "9F1D"
EXPECTED_APPLIED = 270
MAX_CELLS = 20
ROM_SIZE = 16_777_216
SAVE_SIZE = 32_768
SCREENSHOT = ("5CBBF4", "같은　상대　２회　연속　공격")
...
def verify_catalog(rom: bytes) -> None:
    ...
    for row in rows:
        ...
        req(actual == expected, f"{address} render mismatch: {actual!r}")
        req(len(actual) <= MAX_CELLS, f"{address} still over width: {len(actual)}")
        ...
        if address == SCREENSHOT[0]:
            req(actual == SCREENSHOT[1], f"screenshot phrase drifted: {actual!r}")
            seen_shot = True
    req(seen_shot, "screenshot record 5CBBF4 missing")
...
def main() -> int:
    ...
    req(int(audit.get("max_after_cells") or 99) <= MAX_CELLS, "audit width drifted")
    ...
    backup_dir = PATCH / "backup" / f"{stamp}_pre_id_command_effect_width"
    ...
    atomic_json(
        ...
        {
            "schema_version": 1,
            "generated_by": "tools/promote_id_command_effect_width_candidate.py",
            "reason": "pre_id_command_effect_width",
    ...
    report = {
        ...
        "status": "promoted_id_command_effect_width",
        ...
        "screenshot_abs": SCREENSHOT[0],
        "screenshot_after": SCREENSHOT[1],
        ...
        "notes": [
            "Compacted 270 ID-command effect help lines to <=20 cells",
            "Japanese originals in this HUD are already <=20 cells",
            "Combined budget is 소모 + 2-digit SP + space + effect = 25 cells",
