#!/usr/bin/env python3
"""Round-2 terminology unify on the current test ROM.

Parent is ``term_unify_militia_the_o_candidate.wsc``.  Live main TIP is not
modified.  Record bodies stay byte-identical.

User-confirmed this round:
* 레일라 / 레이먼드 → 레이라 레이몬드
* 수오노 → 스오노 (・/공백은 문맥 유지)
* 디・오 이름 vs 디　오 대사는 유지
* 윙/갓 공백은 이미 공백이 있는 슬롯만 유지. 도감 붙여쓰기는 레코드 본문
  고정·inplace 성장 거부로 이번 라운드에서 바꾸지 않는다.
"""
...
PARENT_SAVE = ROOT / "sram/term_unify_militia_the_o_candidate.sav"
...
OUT_ROM = ROOT / "out/patch/term_unify_round2_candidate.wsc"
OUT_SAVE = ROOT / "sram/term_unify_round2_candidate.sav"
OUT_REPORT = ROOT / "out/patch/term_unify_round2_candidate_report.json"

EXPECTED_PARENT = "94598f405e0c6e804e61550deeb07e3ef7a92143718738fd41beebed0762e64d"
EXPECTED_MAIN = "5d3cb79f7d4b5f2674f6a28c2c8033f31d52e5b88b6ca579b033fe6701905229"
ROM_SIZE = 16_777_216
SAVE_SIZE = 32_768
NEEDLES = ("레일라", "레이먼드", "수오노")
FORBIDDEN_AFTER = NEEDLES
SCAN_RANGES = (
    (0x590000, 0x5A0000),
    (0x5C0000, 0x5C7900),
    (0x75C000, 0x75E800),
...
def try_encode(text: str, tbl: Tbl) -> bytes | None:
    payload = try_encode_ko_text(
...
def collect_plan(dictionary, tbl: Tbl) -> tuple[dict[int, tuple[str, str]], dict[int, tuple[str, str]], list[dict[str, Any]]]:
    stock: dict[int, tuple[str, str]] = {}
    ext3: dict[int, tuple[str, str]] = {}
    skipped: list[dict[str, Any]] = []
    indexes = [(idx, "stock") for idx in range(dictionary.count)]
    indexes.extend((idx, "ext3") for idx in range(0x1000, 0x1000 + dictionary.ext3_count))
    for idx, kind in indexes:
        try:
            text = strip_pad(dictionary.expand_index(idx, tbl))
        except Exception:
            continue
        ...
        flat_before = try_encode(text, tbl)
        encoded_after = encode(after, tbl, old_raw=raw, dictionary=dictionary)
        nested = flat_before is not None and len(flat_before) > len(raw)
        grows = len(encoded_after) > len(raw)
        row = {
            "index": f"{idx:05X}",
            "kind": kind,
            "before": text,
        ...
        if nested or grows:
            skipped.append(row)
            continue
        target = stock if kind == "stock" else ext3
        target[idx] = (text, after)
    return stock, ext3, skipped
...
def main() -> int:
    ...
    stock, ext3, skipped = collect_plan(dictionary, tbl)
    if not stock and not ext3:
        raise BuildError("no writable terminology slots found")
    ...
    def queue(index: int, before: str, after: str, kind: str, proof: dict[str, Any]) -> None:
        ...
        interior = proof.get("physical_interior_pointer_entries") or proof.get("interior_pointer_indices") or []
        if interior:
            raise BuildError(f"interior pointer at {index:05X}: {proof}")
        if len(encoded) > int(proof["old_len"]):
            raise BuildError(f"in-place growth refused at {index:05X}")
        entry_abs = int(proof["entry_abs"])
        if any(row["entry_abs"] == entry_abs for row in planned):
            return
        proof = dict(proof)
        proof["ok"] = True
    ...
    outcome = guard_slot_writes(
        ...
        justification=(
            "Terminology unify round 2 on already-live name75/aux/script phrases. "
            "Keepers are current consumers. Confirmed: 레일라→레이라, 레이먼드→레이몬드, "
            "수오노→스오노."
        ),
    )
    if not outcome.ok:
    ...
    for row in planned:
        allow.append(inplace_phrase(candidate, row["proof"], slot_payload[int(row["index"], 16)]))
    ...
    for start, end in SCAN_RANGES:
        for logical, _orig, _kind in _walk_zstring_range(
            ...
            for bad in FORBIDDEN_AFTER:
                if bad in text:
                    leftover.append(
                        {"abs": f"{logical:06X}", "bad": bad, "text": strip_pad(text)[:60]}
                    )
    if leftover:
